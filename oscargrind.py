...
numofHands = int(input("Please enter the number of hands that each gambler has to play before leaving the table. (suggestion: 50)\n"))
...
for i in tqdm(range(numofGambler)):
#--------one round begin-----------
    ...
    while startingbalance>=unit and rounds<=numofHands:
        ...
        if not isauto:
            ...
            if not lasthandwin:
                ...
                if startingbalance<goal:
                    ...
                    #     multiplier = startingbalance//initialbalance
                    #     unit = int(initialbalance*goalrate*min(1,multiplier))  #goal is 1% for each session
                    #     #lastbet = unit
                    #     goal = startingbalance + unit
                    #     bet = unit
                    # else:
        ...
        # else:
        #     time.sleep(dealingSpeed)
    dictofdifftrial.append(balancechange)

    #--------one round end-----------


x = [i for i in range(len(balancechange))]
fig, ax = plt.subplots(figsize=(8, 6))

rreturn = [i[-1]/10000-1 for i in dictofdifftrial]
sumofreturn = 0
winss = 0
for i in rreturn:
    sumofreturn += i
